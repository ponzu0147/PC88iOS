...
struct JPInstruction: Z80Instruction {
    ...
    func execute(cpu: Z80CPU, registers: inout Z80Registers, memory: MemoryAccessing, inputOutput: IOAccessing) -> Int {
        if checkCondition(condition, registers: registers) {
            ...
            registers.regPC = registers.regPC &+ size
            return cycles
        }
    }
...
struct JRInstruction: Z80Instruction {
    ...
    func execute(cpu: Z80CPU, registers: inout Z80Registers, memory: MemoryAccessing, inputOutput: IOAccessing) -> Int {
        if checkCondition(condition, registers: registers) {
            registers.regPC = UInt16(Int(registers.regPC) + Int(offset) + 2)
...
struct CALLInstruction: Z80Instruction {
    ...
    func execute(cpu: Z80CPU, registers: inout Z80Registers, memory: MemoryAccessing, inputOutput: IOAccessing) -> Int {
        if checkCondition(condition, registers: registers) {
            let returnAddress = registers.regPC &+ size
            
            registers.regSP = registers.regSP &- 2
            
            memory.writeWord(returnAddress, at: registers.regSP)
...
struct RETInstruction: Z80Instruction {
    ...
    func execute(cpu: Z80CPU, registers: inout Z80Registers, memory: MemoryAccessing, inputOutput: IOAccessing) -> Int {
        if checkCondition(condition, registers: registers) {
            let returnAddress = memory.readWord(at: registers.regSP)
            
            registers.regSP = registers.regSP &+ 2
            
            registers.regPC = returnAddress
...
struct RSTInstruction: Z80Instruction {
    ...
    func execute(cpu: Z80CPU, registers: inout Z80Registers, memory: MemoryAccessing, inputOutput: IOAccessing) -> Int {
        ...
        registers.regPC = UInt16(address)
